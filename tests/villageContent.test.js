...
describe('David\'s specifics are honored', () => {
  ...
  it('world map: China and US open into his cities; the rest are single stops, ending with Next stop...', () => {
    ...
    expect(cn).toEqual(['北京', '天津', '合肥', '芜湖', '徐州', '上海', '重庆', '成都', '厦门', '济南', '三亚', '广州', '武夷山', '新疆', '内蒙', '武汉', '西安', '苏州', '杭州', '长沙', '延边', '香港'])
    expect(stops[0].places.every(p => p.photos.length > 0)).toBe(true)
    expect(stops[0].places[0].photos).toHaveLength(16)
  ...
  it('the picture board is locked behind the birthday hint, photos to come', () => {
    ...
    expect(real.home.board.sections[3].photos).toHaveLength(54)
