...
try {
  ...
  // world map: locked behind the six-digit album code; then China → cities → 北京
  await page.evaluate(() => localStorage.removeItem('davidworld:album-open'))
  const wmGate = await openZoneByKeys(2, 4, 'ArrowLeft')
  if (wmGate) {
    const gt0 = await wmGate.innerText()
    check(/WORLD MAP/.test(gt0) && /six digits/.test(gt0) && !/130915/.test(gt0), 'world map: locked, hint only')
    for (let i = 0; i < 6; i++) await page.fill(`.v-gate .v-code input[data-d="${i}"]`, '130915'[i])
    await page.click('[data-try-gate]')
    await page.waitForTimeout(300)
  } else check(false, 'world map gate opens')
  const wmPanel = await page.$('.v-worldmap')
  if (wmPanel) {
    ...
    t = await wmPanel.innerText()
    check(/growing up/.test(t) && (await page.$$('.v-wm .v-photo img')).length === 16, 'world map: 北京 shows its sixteen photos')
    ...
    check(nzPhotos.length === 11 && nzPhotos.every(Boolean), `world map: all eleven New Zealand photos render (${nzPhotos.filter(Boolean).length}/11)`)
    await page.click('.v-wm [data-wm-back]')
  ...
  if (pbGate) {
    ...
    check(/six digits/.test(gt) && !/130915/.test(gt) && !/0716/.test(gt), 'picture board: locked, hint only')
    for (let i = 0; i < 6; i++) await page.fill(`.v-gate .v-code input[data-d="${i}"]`, '071600'[i])
    await page.click('[data-try-gate]')
    ...
    check(/Picture board/.test(opened) && /Middle School/.test(opened) && /Future/.test(opened), 'picture board: the album code opens it to its sections')
    ...
    await page.waitForTimeout(1500)
    const lifePhotos = await page.$$eval('.v-pb .v-photo img', els => els.map(i => !i.complete || i.naturalWidth > 0))
    check(lifePhotos.length === 54 && lifePhotos.every(Boolean), `picture board: all 54 Life photos present, none broken (${lifePhotos.filter(Boolean).length}/54)`)
    await page.click('.v-pb [data-pb-back]')
    ...
    await page.waitForTimeout(1500)
    const famPhotos = await page.$$eval('.v-pb .v-photo img', els => els.map(i => !i.complete || i.naturalWidth > 0))
    check(famPhotos.length === 35 && famPhotos.every(Boolean), `picture board: all 35 Family & Friend photos present, none broken (${famPhotos.filter(Boolean).length}/35)`)
    await closePanel()
